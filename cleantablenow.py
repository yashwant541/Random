...
# Processing mode
PROCESSING_MODE = "batch"  # "batch" or "interactive"
...
def read_excel_from_dataiku(filename: str) -> Dict[str, pd.DataFrame]:
    ...
    Uses appropriate engine based on file extension
    """
    folder = get_input_folder()
    ...
    try:
        ...
        try:
            ...
            print(f"   ⚠️ Error creating ExcelFile: {e}")
        ...
        for sheet in sheet_names:
            try:
                ...
                excel_file.seek(0)
                if engine:
                    df = pd.read_excel(excel_file, sheet_name=sheet, engine=engine)
                else:
                    df = pd.read_excel(excel_file, sheet_name=sheet)
                tables[sheet] = df
                print(f"   📊 Sheet '{sheet}': {len(df):,} rows, {len(df.columns)} columns")
            except Exception as e:
                print(f"   ⚠️ Error loading sheet '{sheet}': {str(e)[:100]}")
...
def save_excel_to_dataiku(tables: Dict[str, pd.DataFrame], filename: str) -> None:
    ...
    Always uses openpyxl for output (xlsx format)
    """
...
def parse_regular_numbers(regular_numbers_str: str) -> List[str]:
    ...
    if ',' in cleaned and "'" not in cleaned and '"' not in cleaned:
        numbers = [n.strip() for n in cleaned.split(',') if n.strip()]
        return numbers
...
def process_single_table(table_df: pd.DataFrame, table_num: int) -> pd.DataFrame:
    ...
    if len(structured_df) > 0:
        ...
        if value_cols:
            ...
            print(f"    Sample: '{sample_label[:50]}...' → '{sample_values}'")
...
def save_structured_tables_to_dataiku(structured_tables: Dict[str, pd.DataFrame], output_filename: str) -> None:
    ...
    for sheet_name, table_df in structured_tables.items():
        ...
        summary_data.append({
            ...
            'Sample_Line_Item': (table_df['Line Item'].iloc[0][:50] + '...' 
                                if len(table_df['Line Item'].iloc[0]) > 50 
                                else table_df['Line Item'].iloc[0]) if len(table_df) > 0 else '',
...
def batch_process_all_files():
    ...
    if errors:
        ...
        for result in errors:
            print(f"   • {result['input_file']}: {result['error'][:100]}...")
    
    return results

# =============================================================================
# SIMPLE VERSION FOR DATAiku
# =============================================================================

def simple_process():
    """
    Simple version optimized for Dataiku with proper Excel engine handling
    """
    ...
    for filename in excel_files:
        ...
        try:
            ...
            try:
                if engine:
                    df = pd.read_excel(excel_file, sheet_name='Table_1', engine=engine)
                else:
...
def run():
    ...
    try:
        ...
        try:
            ...
            print("   Please add 'pandas' to your Dataiku environment")
            return
        ...
        try:
            ...
            print("⚠️ openpyxl is not installed (needed for .xlsx files)")
        
        try:
            import xlrd
            print("✅ xlrd is available")
        except ImportError:
            print("⚠️ xlrd is not installed (needed for .xls files)")
        
        print(f"\n📁 Input folder: {INPUT_FOLDER_ID}")
